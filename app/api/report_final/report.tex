...
\par We also tested individual components of the system so as to find possible bugs in the implemention.
  ...
  \par We developed unit tests to verify if the behaviour of the microservices: "anime", "book" and "imdb" - were in accordance with the system specifications and to ensure that access to the respective databases were returning the desired values.
We tested each functions and covered situations in which the objects in use did/didn't exist. We also verified the behaviour of the functions when receiving invalid input (for example, searching for an object with an id that doesn't  exist). Most of the tested operations present the expected behaviour but there were some cases in which the functions didn't properly deal with exceptions (these bugs were fixed).
