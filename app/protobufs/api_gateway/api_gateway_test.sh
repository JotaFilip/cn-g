...
for i in $(seq 0 $retries); do
    ...
    res_comandocriarlivro=$(curl -X POST -u admin:admin --header  'Content-Type: application/json' -d '{"category": [ { "name": "Test"  } ], "description": "string", "name": "Test", "photoUrl": "test", "rating": 4.55,"type": "BOOK"  }' 'https://localhost:5000/item')
...
for i in $(seq 0 $retries); do
    ...
    res_comandocriarlivro=$(curl -X POST -u saldanha:saldanha --header  'Content-Type: application/json' -d '{"category": [ { "name": "Test"  } ], "description": "string", "name": "Test", "photoUrl": "test", "rating": 4.55,"type": "BOOK"  }' 'https://localhost:5000/item')
